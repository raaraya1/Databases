...
class MechanicalSoup_st:
    ...
    def load_page(self, url):
        ...
        if html_code: st.code(str(browser.get_current_page()))
    ...
    def filter_content(self, data_type='p'):
        ...
        page_content = str(browser.get_current_page())
        ...
        content_filtered_1 = [str(i) for i in page.find_all(data_type)]
        ...
        if transform_to_text:
            if second_filter:
                content = page.find_all(data_type)
                result = []
                for item in content:
                    h = str(item).split('>')[0]
                    x = None
