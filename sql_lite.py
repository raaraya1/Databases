...
class sqlite():
    ...
    def fix_primary_key_(self, nombre_tabla, nombre_columna):
        # Nota: para corroborar el cambio usar PRAGMA table_info([nombre_tabla]);
        # 1) pasamos la tabla a df
        comando = f'SELECT * FROM {nombre_tabla}'
        ...
        # 3) pasamos el df a tabla
        with sqlite3.connect(self.path) as conn:
            df.to_sql(nombre_tabla, con=conn, dtype={f'{nombre_columna}': 'VARCHAR(50) PRIMARY KEY'}, index=False)
    ...
    def primary_key(self, nombre_tabla, nombre_columna, num_columna, columna_adicional=False, columnas_generadoras=[]):
        ...
        # caso 2 -> no existen valores duplicados y sin columna adicional
            ...
            self.fix_primary_key_(nombre_tabla, nombre_columna)
    ...
    def foreign_key(self, tabla1, columna1, tabla2, columna2):
        # NOTA: Podemos verificar los cambios ejecutando SELECT tbl_name, sql FROM sqlite_master WHERE sql LIKE ('%REFERENCES%');
        ...
        # copiar tabla1
        comando = f'SELECT * FROM {tabla1}'
        df1 = self.mostrar_tabla(comando, show=False)
        col1 = df1.columns

        # guardar copia
        comando = f'ALTER TABLE {tabla1} RENAME TO old_{tabla1};'
        self.ejecutar(comando)

        # copiar tabla2
        comando = f'SELECT * FROM {tabla2}'
        df2 = self.mostrar_tabla(comando, show=False)

        # Recostruimos la tabla1
        comando = f'''
        CREATE TABLE {tabla1}
        ('''
        for col in col1:
            comando += f'{col} VARCHAR, '
        comando += f'''CONSTRAINT fk_{tabla1}_{tabla2}
        ...
        comando = f'INSERT INTO {tabla1} SELECT * FROM old_{tabla1};'
        self.ejecutar(comando)

        # eliminamos la antigua Tabla1
        comando = f'drop table old_{tabla1}'
        self.ejecutar(comando)
